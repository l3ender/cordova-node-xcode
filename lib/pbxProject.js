...
pbxProject.prototype.addFramework = function(fpath, opt) {
    ...
    if (customFramework) {
        ...
        if (embed) {
          ...
          if (file.target) {
            const targetEmbedFrameworksBuildPhase = this.pbxEmbedFrameworksBuildPhaseObj(embeddedFile.target);
            if (!targetEmbedFrameworksBuildPhase) {
                this.addBuildPhase([], 'PBXCopyFilesBuildPhase', 'Embed Frameworks', file.target, 'frameworks');
            }
        }
...
pbxProject.prototype.addTarget = function(name, type, subfolder, bundleId) {
    ...
    if (targetType === 'app_extension') {
        ...
        this.addBuildPhase([], 'PBXCopyFilesBuildPhase', 'Copy Files', firstTarget.uuid, targetType);

        // Add product to CopyFiles phase
        let extensionFile = Object.assign({}, productFile);
        extensionFile.target = firstTarget.uuid;
        this.addToPbxCopyfilesBuildPhase(extensionFile);
